<template>
	<view>
		<view class="mglr4 pdt15 color9 fs13 center">您选择支付宝转账进行红包提现</view>
		<view class="editLine">
		...
		methods: {
			...
			submit() {
				...
				if (pass) {
					...
					uni.setStorageSync('canClick', true);
					self.$Utils.showToast('请输入支付宝账号或提现金额', 'none')
				};
			},
			...
			getUserInfoData() {
				...
				const callback = (res) => {
					if (res.solely_code == 100000 && res.info.data[0]) {
						...
						self.submitData.zfb = self.userInfoData.trade_info;
					} else {
			...
			userInfoUpdate() {
				...
				postData.data = {
					name:self.submitData.name,
					zfb:self.submitData.trade_info
				};
				const callback = (data) => {				
					if (data.solely_code == 100000) {
						uni.setStorageSync('canClick', true);
