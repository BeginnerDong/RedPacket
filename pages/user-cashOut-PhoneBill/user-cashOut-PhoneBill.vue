<template>
	<view>
		...
		<view class="submitbtn" style="margin-top:150rpx;">
			...
			<view class="w80 fs12 color9">您提交成功后，提现金额会以话费的形式充值到您的手机号上</view>
		</view>
		...
		methods: {
			...
			submit() {
				...
				if (pass) {
					...
					uni.setStorageSync('canClick', true);
					self.$Utils.showToast('请输入手机号或提现金额', 'none')
				};
			},
			...
			getUserInfoData() {
				...
				const callback = (res) => {
					if (res.solely_code == 100000 && res.info.data[0]) {
						...
						self.submitData.phone = self.userInfoData.trade_info;
					} else {
			...
			userInfoUpdate() {
				...
				postData.data = {
					name:self.submitData.name,
					phone:self.submitData.trade_info
				};
				const callback = (data) => {				
					if (data.solely_code == 100000) {
						uni.setStorageSync('canClick', true);
